use crate::storage_types::{DataKey, DeCent-PayError, INSTANCE_BUMP_AMOUNT, INSTANCE_LIFETIME_THRESHOLD};
...
pub fn initialize(env: &Env, owner: Address, fee_collector: Address, platform_fee_bp: u32) -> Result<(), Error> {
    ...
    if env.storage().instance().has(&DataKey::Owner) {
        return Err(Error::from_contract_error(DeCent-PayError::AlreadyInitialized as u32));
    }

    // Validate parameters
    if platform_fee_bp > 1000 {
        // Max 10% (1000 basis points)
        return Err(Error::from_contract_error(DeCent-PayError::FeeTooHigh as u32));
    }
...
pub fn get_owner(env: &Env) -> Result<Address, Error> {
    env.storage()
        .instance()
        .get(&DataKey::Owner)
        .ok_or_else(|| Error::from_contract_error(DeCent-PayError::NotInitialized as u32))
}
